...
angle1_limit_2 = [0,135]
angle2_limit_2 = [-90,0]
angle3_limit_2 = [-45,45]
angle4_limit_2 = [0,135]
angle5_limit_2 = [-90,90]
angle6_limit_2 = [-45,45]
angle7_limit_2 = [-45,45]

#score_weight = [1,0.001,0.001]
score_weight = [0.1,0,0.001]
loop_amount = 10000
prefix_file_name = "fixedangle7_11_solve_w2_"
prefix_file_name_2 = "fixedangle7_7.5_solve_w2_Avg_"
...
for i in range (0,loop_amount):
    ...
    T_Matrix_2 = calKinematicNamo(random_angle_2,'R')

    Q = calQuaternion(T_Matrix[7])
    Q_2 = calQuaternion(T_Matrix_2[7])
    ...
    for j in range(0,len(original_posture_config)):
        ...
        Q_diff_2.append([Q_ref[j][0]-Q_2[0],Q_ref[j][1]-Q_2[1],Q_ref[j][2]-Q_2[2],Q_ref[j][3]-Q_2[3]])

        norm_Q_diff.append(math.sqrt((Q_diff[j][0]*Q_diff[j][0])+(Q_diff[j][1]*Q_diff[j][1])+(Q_diff[j][2]*Q_diff[j][2])+(Q_diff[j][3]*Q_diff[j][3])))
        norm_Q_diff_2.append(math.sqrt((Q_diff_2[j][0]*Q_diff_2[j][0])+(Q_diff_2[j][1]*Q_diff_2[j][1])+(Q_diff_2[j][2]*Q_diff_2[j][2])+(Q_diff_2[j][3]*Q_diff_2[j][3])))
        ...
        elbow_diff_2.append([TMatrix_Target_Posture[j][3][0,3] - T_Matrix_2[3][0,3],TMatrix_Target_Posture[j][3][1,3] - T_Matrix_2[3][1,3],TMatrix_Target_Posture[j][3][2,3] - T_Matrix_2[3][2,3]])
        ...
        norm_elbow_diff_2.append(math.sqrt(pow(TMatrix_Target_Posture[j][3][0,3] - T_Matrix_2[3][0,3],2)+pow(TMatrix_Target_Posture[j][3][1,3] - T_Matrix_2[3][1,3],2)+pow(TMatrix_Target_Posture[j][3][2,3] - T_Matrix_2[3][2,3],2)))
        ...
        wrist_diff_2.append([TMatrix_Target_Posture[j][4][0,3] - T_Matrix_2[4][0,3],TMatrix_Target_Posture[j][4][1,3] - T_Matrix_2[4][1,3],TMatrix_Target_Posture[j][4][2,3] - T_Matrix_2[4][2,3]])
        ...
        norm_wrist_diff_2.append(math.sqrt(pow(TMatrix_Target_Posture[j][4][0,3] - T_Matrix_2[4][0,3],2)+pow(TMatrix_Target_Posture[j][4][1,3] - T_Matrix_2[4][1,3],2)+pow(TMatrix_Target_Posture[j][4][2,3] - T_Matrix_2[4][2,3],2)))

        sum_diff.append(norm_Q_diff[j]*score_weight[0] +   norm_elbow_diff[j]*score_weight[1] +   norm_wrist_diff[j]*score_weight[2])
        sum_diff_2.append(norm_Q_diff_2[j]*score_weight[0] +   norm_elbow_diff_2[j]*score_weight[1] +   norm_wrist_diff_2[j]*score_weight[2])
        ...
        dataAdd_2=[random_angle_2[0],random_angle_2[1],random_angle_2[2],random_angle_2[3],random_angle_2[4],random_angle_2[5],random_angle_2[6],
                 norm_Q_diff_2[j], norm_elbow_diff_2[j], norm_wrist_diff_2[j],sum_diff_2[j]]

        dataSet[j].append(dataAdd)
        diff_all_post.append(sum_diff[j])
        dataSet_2[j].append(dataAdd_2)
        diff_all_post_2.append(sum_diff_2[j])
    ...
    dataAdd_to_All_2 = [random_angle_2[0],random_angle_2[1],random_angle_2[2],random_angle_2[3],random_angle_2[4],random_angle_2[5],random_angle_2[6],
                        diff_all_post_2,sum(diff_all_post_2)]
    dataAll_2.append(dataAdd_to_All_2)
...
saveDataToFile(str(prefix_file_name_2)+"7DOF_Q_All_2.txt",dataAll_2)
dataAll_2.sort(key=itemgetter(8))
saveDataToFile(str(prefix_file_name_2)+"7DOF_Q_All_sorted_2.txt",dataAll_2)

for i in range(0,len(original_posture_config)):
    saveDataToFile(str(prefix_file_name)+"7DOF_Q" + str(i) +".txt",dataSet[i])
    saveDataToFile(str(prefix_file_name_2)+"7DOF_Q_2" + str(i) +".txt",dataSet_2[i])

    dataSet[i].sort(key=itemgetter(10))
    dataSet_2[i].sort(key=itemgetter(10))

    saveDataToFile(str(prefix_file_name)+"7DOF_Q" + str(i) +"_sorted.txt",dataSet[i])
    saveDataToFile(str(prefix_file_name_2)+"7DOF_Q_2" + str(i) +"_sorted.txt",dataSet_2[i])
